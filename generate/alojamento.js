...
function gen(){
    ...
    for(let i = 0; i < alojamentos.length; i++){
        ...
        if(alojamentos[i].fields.zipcode){
            if(alojamentos[i].fields.zipcode.search(" ") >= 0 || alojamentos[i].fields.zipcode.search("-") >= 0){
                codigo_indicativo = alojamentos[i].fields.zipcode.search(" ") >= 0 ? alojamentos[i].fields.zipcode.substring(alojamentos[i].fields.zipcode.search(" ")+1, alojamentos[i].fields.zipcode.length) : alojamentos[i].fields.zipcode.substring(alojamentos[i].fields.zipcode.search("-")+1, alojamentos[i].fields.zipcode.length)
                codigo_cidade = alojamentos[i].fields.zipcode.search(" ") >= 0 ? alojamentos[i].fields.zipcode.substring(0, alojamentos[i].fields.zipcode.search(" ")) : alojamentos[i].fields.zipcode.substring(0, alojamentos[i].fields.zipcode.search("-"))
            }else{
                codigo_cidade = alojamentos[i].fields.zipcode
                codigo_indicativo = alojamentos[i].fields.zipcode
            }
        }else{
            codigo_cidade = 'dne'
    ...
    id_tipo_alojamento: alojamentos[i].fields.room_type, cidade: alojamentos[i].fields.city, pais: alojamentos[i].fields.country})
    }
    return alojamento
}
